...
import { FiPlus, FiTrash2, FiArrowUp, FiArrowDown, FiUpload, FiLink, FiX } from "react-icons/fi"
...
export default function CreatePage() {
  ...
  // --- Detect // inside Quill editor (two-keystroke buffer method) ---
  useEffect(() => {
    let checkInterval
  ...
  async function handleImageUpload(e) {
    ...
    if (!file || !title) return alert("Enter a title before uploading an image.")
  ...
  async function handleSave() {
    ...
    const infoObj = Object.fromEntries(
      info.map(([k], i) => [k, fieldRefs.current[i]?.innerHTML || ""])
    )
  ...
  return (
    <div className="page-container p-4 md:p-6 max-w-5xl mx-auto flex flex-col gap-6 text-gray-900 dark:text-gray-100">
      ...
      <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md p-4">
        ...
        {info.map(([key, value], i) => (
          <div
            key={i}
            className="flex flex-col md:flex-row items-start gap-2 mb-2 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-2 rounded-md"
          >
            <input
      ...
      {/* Content Editor */}
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 min-h-[300px]">
        <ReactQuill
          ref={quillRef}
          value={content}
          onChange={setContent}
          modules={quillModules}
          className="min-h-[300px] dark:text-gray-100"
        />
      </div>

      {/* Page Settings */}
      <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md p-4 space-y-4">
        <h2 className="text-xl font-semibold mb-2">Page Settings</h2>
        <div>
